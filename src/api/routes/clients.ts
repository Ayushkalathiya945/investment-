...
clientRouter.post("/analytics", zValidator("json", clientFilterSchema), async (c) => {
    ...
    try {
        const totalClient = await clientQueries.calculateTotalClient({ from: body.from, to: body.to });
        const totalTradeAmount = await tradeQueries.calculateTotalTradeAmount({ from: body.from, to: body.to });
        const totalBrokerageAmount = await brokerageQueries.calculateTotalbrokerageAmount({ from: body.from, to: body.to });
        const totalPaymentAmount = await paymentQueries.calculateTotalPaymentAmount({ from: body.from, to: body.to });
